...
const CategoryCard = ({ icon: Icon, title, count, isActive, category }) => {
  const navigate = useNavigate();

  return (
    <div 
      onClick={() => navigate(`/jobs?category=${category}`)}
      className={`p-8 border border-gray-100 hover:shadow-2xl transition-all cursor-pointer group rounded-2xl ${isActive ? 'bg-[#4540DB] text-white border-[#4540DB]' : 'bg-white text-[#202430]'}`}
    >
...
const CategorySection = () => {
  const categories = [
    { icon: Palette, title: 'Design', count: 235, category: 'Design' },
    { icon: BarChart3, title: 'Sales', count: 756, category: 'Sales' },
    { icon: Megaphone, title: 'Marketing', count: 140, category: 'Marketing', isActive: true },
    { icon: DollarSign, title: 'Finance', count: 325, category: 'Finance' },
    { icon: Monitor, title: 'Technology', count: 436, category: 'Technology' },
    { icon: Briefcase, title: 'Engineering', count: 542, category: 'Engineering' },
    { icon: Briefcase, title: 'Business', count: 211, category: 'Business' },
    { icon: Users2, title: 'Human Resource', count: 346, category: 'Human Resource' }
  ];
